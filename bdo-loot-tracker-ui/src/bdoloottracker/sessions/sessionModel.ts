...
export const sessions = createModel<SessionState>({
  ...
  effects: dispatch => ({
    async getSessions() {
      const response = await axios.get('http://localhost:8080/api/run/sessions');
      dispatch.sessions.setSessions(response.data);
    },
    async createSession(payload) {
      const response = await axios.post('http://localhost:8080/api/run/sessions', payload);
      dispatch.sessions.setCurrentSession(response.data);
    },
    async findSessionById(sessionId) {
      const response = await axios.get(`http://localhost:8080/api/run/sessions/${sessionId}`);
